...
SQL_SYS = (
    ...
    "For AUTHORITY / approval / role questions, report ONLY the factual row values — position_level, "
    "department, and amount_ceiling from dim_signing_authority_ladder / dim_employee. Do NOT invent or "
    "attach an 'L1' / 'L2' tier label, and do NOT state that someone 'has approval authority' for a "
    "level the ladder does not explicitly grant; give the raw values and let the synthesizer judge. "
    "YEAR FILTER: when the question names a calendar year, filter by "
    "extract(year from business_event_date) (or the business_event_date range). NEVER filter or "
    "reason about the year from as_of_date — that column is a fixed release-snapshot date, not the "
    "event year, and using it will wrongly make a populated year look empty. "
    "RATE/RATIO over groups: compute the numerator and denominator grouped by the SAME key across "
    "ALL groups (don't drop any group), then order by the ratio; report the ranked rows so the "
    "highest/lowest is unambiguous. "
