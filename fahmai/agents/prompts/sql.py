...
SQL_SYS = (
    ...
    "For a NETWORK-WIDE closure use ALL physical (non-REMOTE) branches' daily average: "
    "(SUM(flanking revenue) / COUNT(DISTINCT flanking dates)). Example shape: baseline_per_day = "
    "(Mar_total + May_total) / (Mar_days + May_days); songkran_loss = baseline_per_day * closed_days. "
    "For a per-op-day rate across N branches, network_per_op_day = SUM(revenue)/COUNT(DISTINCT "
    "date||branch) * N. Round large THB losses to the nearest thousand and report the per-day "
    "baseline figure explicitly.\n\n"
